...
CREATE TABLE IF NOT EXISTS `Framakollect`.`specialite` (
  ...
  `idAcces` INT NULL,
  PRIMARY KEY (`id_spe`),
  INDEX `fk_specialite_Acces_idx` (`idAcces` ASC),
  CONSTRAINT `fk_specialite_Acces`
    FOREIGN KEY (`idAcces`)
    REFERENCES `Framakollect`.`Acces` (`idAcces`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `Framakollect`.`membre` (
  ...
  `id_spe` INT(11) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_membre_specialite1_idx` (`id_spe` ASC))
ENGINE = MyISAM
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `Framakollect`.`visite` (
  ...
  `PRA_NUM` INT(2) NULL DEFAULT NULL,
